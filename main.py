...
def getCircleAvatar(qq_num, size=(128, 128)):
    ...
    avatar_img = cv2.resize(avatar_img, dsize=size)
...
def get_img(post_data):
    ...
    img[60:60 + avatar_size[0], 270:270 + avatar_size[1], :] = avatar_img

    # add time
    real_time = post_data["post_date"]
    post_time = "投稿时间：" + real_time[0:4] + "/" + real_time[5:7] + "/" + real_time[8:10]
    print(post_time)
    img = cv2ImgAddText(img, post_time, 250, 250, (128, 128, 128), textSize=20)
    ...
    # add split line
    cv2.line(img, (40, 370), (680, 370), (0, 0, 0), thickness=2)
    ...
    img = cv2ImgAddText(img, contact_tel, 100, 980, (0, 0, 255), 22)

    qr_img = getQrcodeImg()
    img[850:1042, 440:632, :] = qr_img
